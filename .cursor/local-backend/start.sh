...
#     -> PostgREST (port 3010)     -- same data API Supabase exposes
...
PSQL=("$PG_BIN/psql" -h "$STATE_DIR" -p "$PGPORT" -U "$PGUSER" -v ON_ERROR_STOP=1 -X -q)
...
if [ ! -s "$PGDATA/PG_VERSION" ]; then
  ...
  rm -rf "$PGDATA"
  mkdir -p "$PGDATA"
  "$PG_BIN/initdb" -D "$PGDATA" -U "$PGUSER" --auth-local=trust --auth-host=trust >/dev/null
  {
    echo "port = $PGPORT"
...
# ---------------------------------------------------------------------------
if ! "$PG_BIN/psql" -h "$STATE_DIR" -p "$PGPORT" -U "$PGUSER" -tAc \
    "SELECT 1 FROM pg_database WHERE datname='$DB_NAME'" postgres | grep -q 1; then
  log "Creating database $DB_NAME"
  "$PG_BIN/createdb" -h "$STATE_DIR" -p "$PGPORT" -U "$PGUSER" "$DB_NAME"
fi
