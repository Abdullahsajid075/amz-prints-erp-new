...
function amz_prints_enqueue_assets() {
	...
	wp_enqueue_style( 'amz-prints-shop', AMZ_PRINTS_URI . '/assets/css/shop.css', array( 'amz-prints-main' ), AMZ_PRINTS_VERSION );
	wp_enqueue_script( 'amz-prints-main', AMZ_PRINTS_URI . '/assets/js/main.js', array(), AMZ_PRINTS_VERSION, true );
	wp_enqueue_script( 'amz-prints-shop', AMZ_PRINTS_URI . '/assets/js/shop.js', array(), AMZ_PRINTS_VERSION, true );
	...
	wp_localize_script( 'amz-prints-shop', 'amzShop', array(
		...
		'homeUrl'         => home_url( '/' ),
		'productsUrl'     => home_url( '/products/' ),
		'cartUrl'         => home_url( '/cart/' ),
		'checkoutUrl'     => home_url( '/checkout/' ),
		'accountUrl'      => home_url( '/account/' ),
		'trackUrl'        => home_url( '/track-order/' ),
		'deliveryCharges' => floatval( amz_prints_mod( 'amz_delivery_charges', 0 ) ),
		'discount'        => 0,
		'policy'          => amz_prints_mod(
			'amz_order_policy',
			'Your order will begin processing after payment confirmation. Please complete the required payment according to the selected payment method. Order processing will start once payment has been verified.'
		),
	) );
}
