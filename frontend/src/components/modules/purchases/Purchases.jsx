...
import { purchasesAPI, vendorsAPI, ordersAPI } from '@/services/api';
...
const PO_STATUS = ['Purchase Order', 'In Transit', 'Received'];
...
const emptyPurchase = {
  ...
  status: 'Purchase Order', paymentStatus: 'Unpaid',
  linkedOrderId: '',
  items: [{ _key: 'i_init', name: '', quantity: 1, rate: 0, unit: 'piece' }],
...
const Purchases = () => {
  ...
  const fetchAll = useCallback(async () => {
    setLoading(true);
    try {
      const [pRes, vRes, oRes] = await Promise.all([purchasesAPI.getAll(), vendorsAPI.getAll(), ordersAPI.getAll()]);
      setPurchases(pRes.data || []); setVendors(vRes.data || []); setOrders(oRes.data || []);
    } catch (err) {
  ...
  const stats = {
    ...
    unpaid: purchases.filter(p => p.paymentStatus !== 'Paid').reduce((s, p) => s + ((p.totalAmount || 0) - (p.paidAmount || 0)), 0)
  };
  ...
  const calcTotal = () => formData.items.reduce((s, i) => s + (i.quantity * i.rate), 0);
  const updateItem = (i, field, value) => { const items = [...formData.items]; items[i] = { ...items[i], [field]: value }; setFormData({ ...formData, items }); };
  const addItem = () => setFormData({ ...formData, items: [...formData.items, { _key: `i_${Date.now()}_${Math.random().toString(36).slice(2, 7)}`, name: '', quantity: 1, rate: 0, unit: 'piece' }] });
  ...
  const handleSave = async (e) => {
    ...
    const payload = { ...formData, vendorName: vendor?.name, totalAmount: calcTotal() };
    try {
      if (editing) { await purchasesAPI.update(editing.id, payload); toast.success('Updated'); }
      else { await purchasesAPI.create(payload); toast.success('Purchase order created'); }
  ...
  const markReceived = async (p) => {
    if (window.confirm('Mark as received? This updates inventory.')) {
      try {
        await purchasesAPI.update(p.id, { ...p, status: 'Received', actualDeliveryDate: new Date().toISOString().split('T')[0] });
  ...
  const statusColor = (s) => ({ 'Purchase Order': 'bg-blue-100 text-blue-800', 'In Transit': 'bg-yellow-100 text-yellow-800', 'Received': 'bg-green-100 text-green-800' }[s] || 'bg-gray-100');
  const paymentColor = (s) => ({ 'Unpaid': 'bg-red-100 text-red-800', 'Partially Paid': 'bg-yellow-100 text-yellow-800', 'Paid': 'bg-green-100 text-green-800' }[s] || 'bg-gray-100');
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b bg-gray-50">
                      ...
                      <th className="text-left py-3 px-3 text-xs uppercase font-semibold text-gray-600">Payment</th>
                    ...
                    {filtered.map(p => {
                      ...
                      return (
                        <tr key={p.id} className="border-b hover:bg-orange-50 transition-colors" data-testid={`purchase-row-${p.id}`}>
                          ...
                          <td className="py-3 px-3"><Badge className={paymentColor(p.paymentStatus)}>{p.paymentStatus}</Badge></td>
              ...
              <div className="space-y-2">
                {formData.items.map((item, i) => (
                  <div key={item._key || item.id || `item-${i}`} className="grid grid-cols-12 gap-2 items-end p-2 border rounded">
                    <div className="col-span-5"><Label className="text-xs">Item</Label><Input value={item.name} onChange={(e) => updateItem(i, 'name', e.target.value)} required /></div>
                    ...
                    <div className="col-span-2"><Label className="text-xs">Rate</Label><Input type="number" step="0.01" min="0" value={item.rate} onChange={(e) => updateItem(i, 'rate', parseFloat(e.target.value) || 0)} /></div>
          ...
          {viewData && (
            <div className="space-y-4">
              <div className="flex justify-between items-start p-3 rounded-lg" style={{ backgroundColor: '#FFF3ED' }}>
                ...
                <div className="space-y-1 text-right"><Badge className={statusColor(viewData.status)}>{viewData.status}</Badge><Badge className={paymentColor(viewData.paymentStatus) + ' block'}>{viewData.paymentStatus}</Badge></div>
              </div>
