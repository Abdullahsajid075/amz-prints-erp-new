...
#   - PostgreSQL (local cluster) + PostgREST as a Supabase-compatible data layer
...
# Service JWT (role "amz_api") signed with the local PostgREST jwt-secret below.
# Local development credentials only — never used against a real Supabase project.
