...
define( 'AMZ_PRINTS_VERSION', '3.20.0' );
...
function amz_prints_maybe_upgrade_pages() {
	if ( get_option( 'amz_prints_pages_ver' ) === '3.20.0' ) {
		return;
	}
	...
	$saved_api = (string) get_theme_mod( 'amz_erp_api_url', '' );
	if ( ! $saved_api || false !== strpos( $saved_api, 'script.google.com' ) ) {
		set_theme_mod( 'amz_erp_api_url', 'https://amz-prints-api.vercel.app' );
	}
	delete_transient( 'amz_prints_erp_products_v3' );
	delete_transient( 'amz_prints_erp_products_v4' );
	update_option( 'amz_prints_pages_ver', '3.20.0' );
}
