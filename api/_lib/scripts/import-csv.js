...
const TABLE_MAPS = {
  customers: (row) => ({
    ...
    blocked: String(row.blocked || '').toLowerCase() === 'true',
    block_reason: row.blockreason || row.BlockReason || '',
    blocked_at: row.blockedat || row.BlockedAt || '',
    blocked_by: row.blockedby || row.BlockedBy || '',
    credit_balance: Number(row.creditbalance || row.CreditBalance || 0),
  }),
  users: (row) => ({
  ...
  products: (row) => ({
    ...
    images: parseMaybeJson(row.images || row.Images || '[]'),
    sale_price: Number(row.saleprice || row.SalePrice || 0),
    show_on_top: String(row.showontop || '').toLowerCase() === 'true',
    image: row.image || row.Image || row.photo || '',
  ...
  invoices: (row) => ({
    ...
    order_id: row.orderid || row.OrderId || '',
    order_ids: parseMaybeJson(row.orderids || row.OrderIds || '[]'),
    customer_id: row.customerid || row.CustomerId || '',
    customer_name: row.customername || row.CustomerName || '',
    customer_phone: row.customerphone || row.CustomerPhone || '',
    customer_email: row.customeremail || row.CustomerEmail || '',
  ...
  expenses: (row) => ({
    ...
    paid_to: row.paidto || row.PaidTo || '',
    notes: row.notes || row.Notes || '',
    approved: String(row.approved || '').toLowerCase() === 'true',
    approved_by: row.approvedby || row.ApprovedBy || '',
    approved_at: row.approvedat || row.ApprovedAt || '',
  }),
  payments: (row) => ({
